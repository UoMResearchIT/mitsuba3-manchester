...
def run_experiment(photon_list):
    ...
    # Number of samples = sample_count * sensor_size (see src/render/integrator.cpp line 575)
    sensor_width = 1024
    sensor_height = 1024
    number_of_photons = (len(np.array(photon_list)[0][0]) - 1) // 6
    sample_count = math.ceil(number_of_photons / (sensor_width * sensor_height))
    print("number of photons, sample_count: ", number_of_photons, sample_count)
